...
void merge_files(const std::vector<std::string>& files, const std::string& output_path)
{
    ...
    if (res.status == merger::MergeStatus::SUCCESS)
    {
        userTextStream << "\n[SUCCESS] Files successfully merged into " << output_path << "\n"
                       ...
                       << "Verified SHA-256: " << res.computed_sha256 << "\n"
    ...
    else if (res.status == merger::MergeStatus::CHECKSUM_MISMATCH)
    {
        userTextStream << "\n[FAILED] Checksum verification failed!\n"
                       << "Computed SHA-256: " << res.computed_sha256 << "\n"
                       << "Expected SHA-256: " << res.expected_sha256 << "\n"
                       << "Output retained for inspection at: " << res.retained_failed_path << "\n"
                       << "Destination " << output_path << " was NOT published.\n";
    }
    else if (res.status == merger::MergeStatus::MANIFEST_NOT_FOUND ||
             res.status == merger::MergeStatus::MANIFEST_INVALID ||
             res.status == merger::MergeStatus::MANIFEST_MISMATCH ||
             res.status == merger::MergeStatus::GEOMETRY_MISMATCH)
    {
        userTextStream << "\n[FAILED] Manifest validation error:\n" << res.error_message << "\n"
                       << "Ensure the .manifest.json generated by the splitter is in /data/pkg_merger.\n";
    }
